...
Provide a CSV or Parquet file with timestamped numeric sensor columns. `label` is optional but required for evaluation. Labels use `0` for normal and `1` for anomaly. A precomputed `score` column can be supplied with `--score-column`; otherwise a robust multivariate score is built from the numeric sensor columns.
...
python /kaggle/input/industrial-alert-calibration/kaggle/run_kaggle.py \
  --input /kaggle/input/YOUR-TELEMETRY/telemetry.csv --run-id experiment-001
```
...
The calibration slice is selected chronologically from the first 30% of the input. Baseline and calibration scores are generated without labels; labels are only used for evaluation. The conformal p-value is the finite-sample upper-tail rank of a score against the frozen calibration distribution. A p-value at or below `alpha` becomes a point alert.
